...
$$
\begin{cases}
H_0: \lambda(u)=\lambda \: \text{La intensidad es constante} \\
H_1: \lambda(u) \neq \lambda \: \text{La intensidad no es constante}
\end{cases}
$$
...
* Los modelos que consideran tendencia tanto en $x$ como en $y$ presentan mejoras al tener una menor escala en los residuales y capturar mejor la tendencia de accidentes en la ciudad.
...
envol_np_pcf<-envelope(datos_ppp,Kinhom,lambda=ppp_np,nsim = 39)
#saveRDS(envol_np_pcf, file = "funciones_segundo_orden/pcf_np.Rds")
#plot(envol_np_pcf)
```
