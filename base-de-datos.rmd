...
ppp_np <- density.ppp(datos_ppp, sigma = bw.scott, isotropic = T)

# pair correlation function parametrica
# wrapper_param <-  function(x, y){
#   predict(log_lin_cuad_inter, 
#           locations = data.frame(x, y),
#           type = "intensity")
# }
# 
# g_param <- envelope(datos_ppp_km, pcfinhom, lambda = wrapper_param)
# saveRDS(g_param, "funciones_segundo_orden/g_param.Rds")
g_param <- readRDS("funciones_segundo_orden/g_param.Rds")

# Envolventes con funcion K
# env_np <- envelope(datos_ppp, Kinhom, lambda = ppp_np, nsim = 39)
# saveRDS(env_np, "funciones_segundo_orden/K_np.Rds")
K_np <- readRDS("funciones_segundo_orden/K_np.Rds")
```
